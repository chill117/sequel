...
var Instance = function(model, data, options, modeler) {
...
_.extend(Instance.prototype, EventEmitter.prototype, {
	...
	create: function(options) {
		...
		this.runHook('beforeCreate').complete(function(error) {
			...
			Query.Insert(data, options).complete(function(error, insert_id) {
	...
	update: function(options) {
		...
		this.runHook('beforeUpdate').complete(function(error) {
			...
			Query.Update(data, options).complete(function(error, num_updated) {
	...
	runHook: function(type) {
		...
		var hooks = this.getHooks(type)
		var data = this.getAllData()

		var self = this

		// The way hooks are run will be changed once migrated away from sequelize.
		// !!!

		async.eachSeries(hooks, function(fn, next) {

			fn.call(undefined, data, function(error, new_values) {

				if (error)
					return next(error)

				if (typeof new_values != 'undefined')
					data = new_values
...
// For now..
// To make migration away from sequelize easier.
Instance.prototype.getDataValue = Instance.prototype.get
Instance.prototype.setDataValue = Instance.prototype.set
