...
_.extend(Instance.prototype, EventEmitter.prototype, {
	...
	hasChanged: function(name) {
		...
		return this.valueMatchesBetweenDataSets(name, this.data, this.syncedData)
	...
	wasChanged: function(name) {
		...
		return this.valueMatchesBetweenDataSets(name, this.syncedData, this.dataBeforeLastSync)

	},

	valueMatchesBetweenDataSets: function(name, data1, data2) {
	...
	getValueFromData: function(name, data) {
		...
		if (data[name] instanceof BigNumber)
			return new BigNumber( data[name].toString() )
