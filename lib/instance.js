...
_.extend(Instance.prototype, EventEmitter.prototype, {
	...
	hasChanged: function(name) {
		...
		for (var i in this.data[name])
			if (
				typeof this.syncedData[name][i] == 'undefined' ||
				this.data[name][i] !== this.syncedData[name][i]
			)
				return true

		if (_.isArray(this.data[name]))
			return this.data[name].length != this.syncedData[name].length
