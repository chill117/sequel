...
_.extend(Instance.prototype, EventEmitter.prototype, {
	...
	get: function(name) {
		...
		if (this.data[name] instanceof BigNumber)
			return new BigNumber( this.data[name].toString() )

		if (_.isDate(this.data[name]))
			return new Date( this.data[name].toString() )
	...
	hasChanged: function(name) {
		...
		if (currentValue !== null && typeof this.syncedData[name] == 'undefined')
			return true
		...
		var oldValue = this.syncedData[name]
		...
		if (field.isArray())
		{
			...
			for (var i in currentValue)
				...
				{
					case 'decimal':
						if (!currentValue[i].equals(oldValue[i]))
							return true
		...
		if (dataType == 'date')
			return currentValue.toString() !== oldValue.toString()
