...
_.extend(Field.prototype, {
		...
		async.each(rules, function(rule, next) {
			...
			switch (typeof fn)
			{
				case 'string':
					...
					if (rule.invert && !(fn = Validator.getInverse(fn)))
					{
						errors.push('Inverse of validator function not found: \'' + fn + '\'')
	...
	prepareValidationRules: function() {
		...
		async.each(_.keys(methods), function(name, next) {
			...
			switch (name)
			{
				// A convenience validation that splits up to 'minLen' and 'maxLen'.
				case 'len':

					var args = methods[name]

					var minLen = args[0]
					var maxLen = args[1] || null
			...
			switch (typeof methods[name])
			{
				...
				case 'boolean':
					...
					rule.fn = name
					rule.invert = methods[name] === false
