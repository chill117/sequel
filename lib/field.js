		...
		async.each(rules, function(rule, next) {
			...
			switch (typeof fn)
			{
				case 'string':
					...
					if (!Validation.testExists(fn))
					{
						errors.push('Validation test not found: \'' + fn + '\'')
					...
					if (!Validation.test(fn, [value].concat(args)))
					{
						var error = rule.msg || Validation.getError(fn, args[0])
