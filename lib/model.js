...
_.extend(Model.prototype, {
	...
	disambiguateOptions: function(options) {
		...
		if (options.order_by && options.order_by.indexOf('.') == -1)
			options.order_by = options.table + '.' + options.order_by

		if (options.group_by && options.group_by.indexOf('.') == -1)
			options.group_by = options.table + '.' + options.group_by
	...
	compileSelectFromOptions: function(options) {
		...
		if (!!options.include && options.include.length > 0)
		{
			...
			{
				var include = options.include, model = {}

				if (include.model)
					model = sequel.getModel(model)
				...
				if (!!include.attributes)
					for (var n in include.attributes)
						select[i] = as + '.' + include.attributes[n]
			}
		}
