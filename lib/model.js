	...
	castWhereClauseDataToTypes: function(where) {
		...
		for (var fieldName in where)
		{
			...
			if (
				...
				typeof where[fieldName] == 'object' &&
				where[fieldName].toString() == '[object Object]'
			)
			{
				for (var operator in where[fieldName])
				{
					where[fieldName][operator] = field.castToDataType(where[fieldName][operator])

					if (dataType == 'decimal')
						where[fieldName][operator] = where[fieldName][operator].toString()
				}
			}
			else
			{
				where[fieldName] = field.castToDataType(where[fieldName])

				if (dataType == 'decimal')
					where[fieldName] = where[fieldName].toString()
			}
		}
