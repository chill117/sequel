...
_.extend(Model.prototype, {
	...
	find: function(primary_key, options) {
		...
		options.select = options.attributes || []

		if (!!options.include)
			options.joins = this.compileJoinsFromInclude(options.include)
	...
	compileJoinsFromInclude: function(include) {
		...
		for (var i in include)
		{
			...
			join.table = include[i].table || ''
			join.on = include[i].on || []
			join.as = include[i].as || join.table
			join.type = include[i].join || ''
