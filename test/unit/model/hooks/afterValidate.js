...
describe('Model#hooks \'afterValidate\'', function() {
	...
	before(function() {
		...
		model = sequel.define('ModelHooksAfterValidate', {
	...
	it('should execute all of the callbacks after validation, when validation succeeded', function(done) {

		model.clearHooks()

		var repeat_n_times = 3, num_called = 0

		var changeValue2 = 6000// To something that would fail validation.

		for (var n = 1; n <= repeat_n_times; n++)
			model.addHook('afterValidate', function(next) {

				this.set('value2', changeValue2)
		...
		model.create(data).complete(function(errors, instance) {
			...
			expect(instance.get('value2')).to.equal(changeValue2)
