...
describe('Instance#wasChanged(name)', function() {
	...
	it('should return FALSE for fields that have not been changed', function() {
		...
		for (var field in data)
			expect(instance.wasChanged(field)).to.equal(false)

	})

	it('should return FALSE for fields that have been changed, before the changes have been saved to the database', function() {
	...
	it('should return TRUE for all the fields that were changed, after the changes have been saved to the database', function(done) {
		...
		instance.save().complete(function(errors, result) {
			...
			for (var field in newData)
				expect(result.wasChanged(field)).to.equal(true)
